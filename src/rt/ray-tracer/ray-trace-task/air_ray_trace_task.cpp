...
void AirRayTraceTask::operator()() {
    ...
    emit_shadow_rays(intersection, surface_brdf);

    if (remaining_recursion_depth_ <= 0) { return; }

    emit_refracted_ray(intersection, surface_brdf);
}

void AirRayTraceTask::emit_shadow_rays(const Intersection & intersection, const BRDFunction & surface_brdf) {
    ...
    for (const LightSource & light_source : scene().enumerate_light_sources()) {
        ...
        ray_tracer_->trace_ray(std::make_unique<ShadowRayTraceTask>(*this,
                                                                    illuminance,
...
void AirRayTraceTask::emit_refracted_ray(const Intersection & intersection, const BRDFunction & surface_brdf) {
    const Vec3 & surface_normal = intersection.normal();
    ...
    ray_tracer_->trace_ray(std::make_unique<AirRayTraceTask>(*this, reflected_ray, incoming_intensity));
}
